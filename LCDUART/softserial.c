...
unsigned char SoftSerial_available(void) {
    int cnt;

    __disable_interrupt(); // multiple asm statments, we need to protect head/tail from RX_ISR
...
int SoftSerial_read(void) {
    ...
    if (rx_buffer.head != rx_buffer.tail) {
        c = (unsigned char)rx_buffer.buffer[rx_buffer.tail];
...
void SoftSerial_xmit(unsigned char byte) {
...
void SoftSerial_TX_ISR(void) {
    static unsigned char txBitCnt = 10;       // 1 Start bit + 8 data bits + 1 stop bit
